-- ============================================================================
-- SILVER TRANSACTION DEVICES (M4) - g3_catalog.silver.transaction_devices
-- ============================================================================
--
-- Purpose:
...
CREATE TABLE IF NOT EXISTS g3_catalog.silver.transaction_devices (
  device_id             STRING,
...
WITH checked AS (
  ...
  FROM g3_catalog.bronze.transaction_devices d
...
INSERT OVERWRITE g3_catalog.silver.transaction_devices
WITH checked AS (
  SELECT
    d.*,
    t.transaction_id AS silver_transaction_id
...
DELETE FROM g3_catalog.gov.masking_policies
WHERE table_name = 'transaction_devices';

INSERT INTO g3_catalog.gov.masking_policies VALUES
...
INSERT INTO g3_catalog.gov.metadata_lineage VALUES
  ('g3_catalog', 'bronze', 'transaction_devices', 'device_id', 'g3_catalog', 'silver', 'transaction_devices', 'device_id', 'Tokenized with salted SHA256 prefix'),
  ('g3_catalog', 'bronze', 'transaction_devices', 'transaction_id', 'g3_catalog', 'silver', 'transaction_devices', 'transaction_id', 'Direct copy after Silver transaction relationship check'),
  ('g3_catalog', 'bronze', 'transaction_devices', 'device_type', 'g3_catalog', 'silver', 'transaction_devices', 'device_type', 'Lowercased and trimmed; missing values quarantined'),
  ('g3_catalog', 'bronze', 'transaction_devices', 'ip', 'g3_catalog', 'silver', 'transaction_devices', 'ip', 'IPv4 reduced to /24-style network; non-IPv4 hashed'),
  ('g3_catalog', 'bronze', 'transaction_devices', 'geo_country', 'g3_catalog', 'silver', 'transaction_devices', 'geo_country', 'Uppercased and trimmed');

SELECT
  'silver.transaction_devices' AS table_name,
  COUNT(*) AS silver_rows
FROM g3_catalog.silver.transaction_devices
UNION ALL
SELECT
