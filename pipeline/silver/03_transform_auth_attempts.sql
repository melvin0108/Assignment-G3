-- ============================================================================
-- SILVER AUTH ATTEMPTS (M4) - g3_test.silver.auth_attempts
-- ============================================================================
--
-- Purpose:
...
CREATE TABLE IF NOT EXISTS g3_test.silver.auth_attempts (
  attempt_id            STRING,
...
WITH checked AS (
  ...
  FROM g3_test.bronze.auth_attempts a
...
INSERT OVERWRITE g3_test.silver.auth_attempts
WITH checked AS (
  SELECT
    a.*,
    try_to_timestamp(replace(replace(a.auth_ts, 'T', ' '), 'Z', '')) AS auth_ts_typed,
...
INSERT INTO g3_test.gov.metadata_lineage VALUES
  ('g3_test', 'bronze', 'auth_attempts', 'attempt_id', 'g3_test', 'silver', 'auth_attempts', 'attempt_id', 'Direct copy'),
  ('g3_test', 'bronze', 'auth_attempts', 'transaction_id', 'g3_test', 'silver', 'auth_attempts', 'transaction_id', 'Direct copy after Silver transaction relationship check'),
  ('g3_test', 'bronze', 'auth_attempts', 'decision', 'g3_test', 'silver', 'auth_attempts', 'decision', 'Lowercased and trimmed'),
  ('g3_test', 'bronze', 'auth_attempts', 'decline_reason', 'g3_test', 'silver', 'auth_attempts', 'decline_reason', 'Trimmed; empty string converted to NULL'),
  ('g3_test', 'bronze', 'auth_attempts', 'auth_ts', 'g3_test', 'silver', 'auth_attempts', 'auth_ts', 'Parsed to TIMESTAMP; invalid or after transaction timestamp quarantined');

SELECT
  'silver.auth_attempts' AS table_name,
  COUNT(*) AS silver_rows
FROM g3_test.silver.auth_attempts
UNION ALL
SELECT
