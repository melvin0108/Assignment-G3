...
# ----------------------------------------------------------------------------
# Lands the raw source CSV for transactions as a string-typed Bronze Delta table
# with the 8 metadata columns (docs/bronze-layer.md S4). Mirrors the
# CREATE TABLE + COPY INTO block for transactions in 01_ingest_bronze.sql.
...
SOURCE_COLS = ["transaction_id", "account_id", "card_id", "merchant_id", "channel", "amount", "currency", "txn_ts", "status"]
RECORD_ID_COLS = ["transaction_id"]
