...
# ============================================================================
# BRONZE INGEST: branches
# ----------------------------------------------------------------------------
# Lands the raw source CSV for branches as a string-typed Bronze Delta table
# with the 8 metadata columns (docs/bronze-layer.md S4). Mirrors the
# CREATE TABLE + COPY INTO block for branches in 01_ingest_bronze.sql.
...
SOURCE_COLS = ["branch_code", "name", "country", "region", "status"]
RECORD_ID_COLS = ["branch_code"]
