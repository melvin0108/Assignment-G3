...
# BRONZE INGEST: cards
# ----------------------------------------------------------------------------
# Lands the raw source CSV for cards as a string-typed Bronze Delta table
# with the 8 metadata columns (docs/bronze-layer.md S4). Mirrors the
# CREATE TABLE + COPY INTO block for cards in 01_ingest_bronze.sql.
...
SOURCE_COLS = ["card_id", "account_id", "card_type", "pan", "expiry", "status"]
RECORD_ID_COLS = ["card_id"]
