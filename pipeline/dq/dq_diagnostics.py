...
-- ============================================================================
-- (5) SCD1 duplicate-rule validation.
--     dq_03 partitions only by business key because dimension history is not
--     retained, and keeps the row with the latest effective_at. These checks
--     report superseded or duplicate keys not present in the manifest.

-- (5a) DQ-CUST-ID-DUP — caught keys NOT explained by a defect.
--      EXPECTED 0 rows for an SCD1 input containing one row per customer_id.
...
-- (5b) DQ-CARD-DUP — same unexpected-duplicate check. EXPECTED 0 rows.
