...
FROM g3_catalog.bronze.merchants
...
FROM g3_catalog.bronze.fraud_alerts
...
FROM g3_catalog.bronze.transaction_devices
...
FROM g3_catalog.bronze.accounts a
LEFT JOIN g3_catalog.bronze.customers c ON a.customer_id = c.customer_id
...
FROM g3_catalog.bronze.disputes d
LEFT JOIN g3_catalog.bronze.transactions t ON d.transaction_id = t.transaction_id
WHERE t.transaction_id IS NULL;
...
FROM g3_catalog.bronze.chargebacks cb
LEFT JOIN g3_catalog.bronze.disputes d ON cb.dispute_id = d.dispute_id
...
FROM g3_catalog.bronze.transaction_devices x
LEFT JOIN g3_catalog.bronze.transactions t ON x.transaction_id = t.transaction_id
WHERE t.transaction_id IS NULL;
...
FROM g3_catalog.bronze.case_transactions ct
LEFT JOIN g3_catalog.bronze.transactions t ON ct.transaction_id = t.transaction_id
WHERE t.transaction_id IS NULL;
...
FROM g3_catalog.bronze.transactions t
JOIN g3_catalog.bronze.cards c ON t.card_id = c.card_id
...
FROM g3_catalog.bronze.case_parties cp
WHERE (cp.party_type = 'customer' AND cp.party_id NOT IN (SELECT customer_id FROM g3_catalog.bronze.customers))
   OR (cp.party_type = 'merchant' AND cp.party_id NOT IN (SELECT merchant_id FROM g3_catalog.bronze.merchants));
...
FROM g3_catalog.bronze.customer_contact_logs cl
...
FROM g3_catalog.bronze.customer_contact_logs
...
FROM g3_catalog.bronze.investigation_notes n
JOIN g3_catalog.bronze.investigation_cases c ON n.case_id = c.case_id
...
FROM (
  ...
  FROM g3_catalog.bronze.defects_manifest GROUP BY rule_id
) m
LEFT JOIN (
