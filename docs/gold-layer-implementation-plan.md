...
Build a current-state Gold fact constellation in the `g3_catalog.gold` schema. All Gold tables will be AI-safe and documented with one YAML contract per model. A materialized `investigation_context` table will be assembled exclusively from these Gold models.
...
- `usage_restrictions STRING` -- `internal_only`

Non-date surrogate keys use deterministic SHA-256 hashes of the model name and business key. Fact keys hash the complete declared grain. `dim_date.date_key` uses `yyyyMMdd`; key `0` represents an unknown date.
...
| `dim_case` | One eligible case: `case_key`, `case_id`, priority, status code/description, fraud type code/description/severity, opened/closed timestamps and date keys. Includes case-level quality and warnings aggregated from its facts. |
| `dim_merchant` | One merchant: `merchant_key`, `merchant_id`, name, MCC, category name/group, country, risk rating, status, effective timestamp, `is_unknown`. |
| `dim_channel` | One channel: `channel_key`, code, name, `is_unknown`. |
| `dim_dispute_reason` | One reason code: `dispute_reason_key`, code, description, `is_unknown`. |
| `dim_currency` | One currency: `currency_key`, code, name, decimals, `is_unknown`. |
...
| Model | Grain and principal fields |
|---|---|
| `fact_case_transaction` | One case-transaction link: fact key, case/merchant/channel/currency/date keys, transaction ID, amount `DECIMAL(18,2)`, timestamp, status. |
| `fact_authorization_attempt` | One case-authorization attempt: fact key, case key, transaction and attempt IDs, authorization date key, decision, decline reason, timestamp. |
| `fact_dispute` | One case-dispute: fact key, case/reason/currency/date keys, transaction and dispute IDs, amount `DECIMAL(18,2)`, status, raised timestamp. |
| `fact_chargeback` | One case-chargeback: fact key, case/currency/date keys, transaction, dispute, and chargeback IDs, scheme, amount `DECIMAL(18,2)`, stage, processed timestamp. |
| `fact_fraud_alert` | One case-fraud alert: fact key, case/date keys, transaction and alert IDs, rule name, score, disposition, triggered timestamp. |
| `fact_investigation_note` | One safe case note: fact key, case/date keys, note ID, PII-screened text, created timestamp. Author identity is excluded. |
| `fact_case_party_summary` | One case/party-type/role combination: fact key, case key, party type, role, party count. Raw party identifiers are excluded. |
...
1. Add shared Gold helpers for catalog validation, snapshot consistency, deterministic keys, unknown members, eligible-case filtering, standard metadata, typed arrays, and overwrite writes.
...
- Dimension and fact keys are non-null, unique at their declared grains, and stable across identical reruns.
...
- Context validation fails if it reads from Silver or exposes Gold surrogate keys.
